...
const PayController = {
    ...
    async return(req, res) {
        ...
        try {
            transaction = await Transaction.findById(req.params.transactionId).exec()
        } catch (err) {
            return res.status(500).json({ message: 'Server error'} )
        }
        ...
        try {
            ...
            const order = await Order.findById(newTransaction.orderId).exec()
            ...
            order.status = 'PAID'
            order.transactionId = newTransaction._id
